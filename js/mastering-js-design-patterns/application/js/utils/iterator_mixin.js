define( [],
	...
	return {
		...
		hasNext: function() {
			var currentChildHasNext = this.currentIndex >= 0
			&& this.currentIndex < this.childTracks.length
			&& this.childTracks[this.currentIndex].hasNext();
			var nextTrackAvailable = this.currentIndex + 1 < this.childTracks.length;
		...
		next: function() {
			if ( this.hasNext() ) {
				...
				if ( currentChildHasNext ) {
					return {
						value: this.childTracks[this.currentIndex].next().value,
						done: false
					}
				} else {
					this.currentIndex++;
					return {
						value: this.childTracks[this.currentIndex],
						done: false
					}
				}
			} else {
		...
		current: function() {
			if (this.currentIndex >= 0 && this.currentIndex < this.childTracks.length) {
				...
				if ( currentTrack && !currentTrack.current().done && currentTrack.current().value) {
					return {
						value: this.childTracks[this.currentIndex].current().value,
						done: false
					}
				} else {
					return {
